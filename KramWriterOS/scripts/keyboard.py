...
# KEYMAPS - All rows MUST have exactly 12 items
_BASE = [
...
GPIO.setwarnings(False) # Clean up terminal output
...
def get_current_key(r, c):
    if active_layers["NUM"] and _NUM[r][c] is not None: return _NUM[r][c]
    if active_layers["SYM"] and _SYM[r][c] is not None: return _SYM[r][c]
    return _BASE[r][c]
    ...
    while True:
        for r_idx, r_pin in enumerate(ROWS):
            ...
            for c_idx, c_pin in enumerate(COLS):
                ...
                # Check mapping for current row/col
                base_val = _BASE[r_idx][c_idx]
                
                # Handle Layer Modifiers
